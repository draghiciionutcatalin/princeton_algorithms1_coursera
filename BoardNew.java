...
public class BoardNew {
    ...
    private List<BoardNew> neighbour = new ArrayList<>();

    // create a board from an n-by-n array of tiles,
    // where tiles[row][col] = tile at (row, col)
    public BoardNew(int[][] tiles) {
        n = tiles[0].length;
        board = new char[n * n];
    ...
    public static void main(String[] args) {
        ...
        BoardNew b = new BoardNew(tiles);
        for (BoardNew b1 : b.neighbors()) {
            System.out.println(b1);
        }
    }

    private BoardNew exch(char[] board, int indexRow, int indexColumn, int i, int indexColumn1) {
        int n1 = n;
        ...
        BoardNew result = new BoardNew(tempboard);
        return result;
    }
    ...
    public int hamming() {
        ...
        for (int i = 0; i < n; i++) {
            for (int j = 0; j < n; j++) {
                if (board[n * i + j] != (char) 0) if (board[n * i + j] != (char) n * i + j + 1) {
                    count++;
                }
            }
        }
    ...
    public Iterable<BoardNew> neighbors() {
