...
public class Percolation {
    ...
    public Percolation(int n) {
        ...
        int[] x = new int[n];
        Arrays.sort(x);
        List<int[]> xa = Arrays.asList(x);
        xa.get(20);
        // Collections.sort(xa.stream().collect(Collectors.toList()));

        String[] xx = "asdasd ads asd asdsa ".split(" ");

        for (String s : xx) {
            System.out.println(s);
        }

        // Iterator<String> stringP = xx.iterator();
        // while (stringP.hasNext()) {
        //   System.out.println(stringP.next());
        //}
    }
